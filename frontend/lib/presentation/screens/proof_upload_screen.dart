...
class _ProofUploadScreenState extends ConsumerState<ProofUploadScreen> {
  ...
  Future<void> _submitProof() async {
    // ✅ Proof files are now optional - users can submit work without files
    ...
    try {
      ...
      if (!isOnline) {
        ...
        await syncService.addToQueue(
          SyncAction.submitProof(
            taskId: widget.taskId,
            beforePaths: _beforeFiles.map((f) => f.path ?? '').toList(),
            afterPaths: _afterFiles.map((f) => f.path ?? '').toList(),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('📦 Queued for sync when online'), backgroundColor: AppTheme.secondaryBlue),
          );
