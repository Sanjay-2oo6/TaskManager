...
import '../../state/filtered_tasks_provider.dart';
...
import '../../presentation/widgets/organization_indicator.dart';
import '../../presentation/widgets/role_based_widget.dart';
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // CRITICAL: Clear badge state from previous session/user to prevent stale badges
      ref.read(unreadCountsProvider.notifier).clearAll();
      print('🧹 Badge state cleared on HomeScreen initialization');
    ...
    try {
      ...
      if (mounted) {
        if (counts.isEmpty) {
          ...
          ref.read(unreadCountsProvider.notifier).clearAll();
          print('✅ Notification Catch-up: No unread messages - all badges cleared');
        } else {
          ...
          print('📊 Unread counts details: $typedCounts');
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final role = widget.user?['role']?.toString().toLowerCase() ?? 'employee';
    ...
    if (isDesktop) {
      ...
      // Load organizations for super admin if needed
      final authState = ref.watch(authNotifierProvider);
      if (authState.isSuperAdmin) {
        ref.watch(organizationProvider.notifier).loadOrganizations();
      }
      ...
      content = Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          actions: [
            // Show organization switcher for super admin
            if (authState.isSuperAdmin)
              Padding(
                padding: const EdgeInsets.only(right: 16.0),
                child: OrganizationSwitcher(
                  onOrganizationSelected: () {
                    ref.refresh(filteredTasksProvider);
                  },
                ),
              ),
          ],
...
class _DashboardOverviewState extends ConsumerState<_DashboardOverview> {
  ...
  Widget _buildRecentTasksSection(List<dynamic> tasks) {
    ...
    final role = widget.user?['role']?.toString().toLowerCase() ?? 'worker';
    final isAdmin = role == 'admin' || role == 'master';
